...
IF OBJECT_ID('dbo.sp_help_permissions') IS NULL
	EXEC ('CREATE PROCEDURE dbo.sp_help_permissions AS --');
GO
...
*/
ALTER PROCEDURE dbo.sp_help_permissions @principal SYSNAME = NULL
...
BEGIN TRY
	...
	IF (@output_xml = 1)
		SET @permission_list = 0;

	DECLARE @is_azure BIT = (
			CASE 
				WHEN CAST(SERVERPROPERTY('Edition') AS VARCHAR(100)) LIKE '%Azure%'
					THEN 1
				ELSE 0
				END
			);
	...
	DECLARE @principals TABLE (
		...
		,PRIMARY KEY CLUSTERED (
			declared_type_desc
			,declared_principal_id
			,effective_type_desc
			,effective_principal_id
			,_id
			)
		);
	DECLARE @d_public INT
		,@sysadmin INT;

	SELECT @d_public = principal_id
...
*/
	...
	BEGIN
        ...
        WHERE [sid] IS NOT NULL' + (
				CASE 
					WHEN @hide_system_principals = 1
						THEN N'
          AND [type]!=''C'' AND UPPER([name]) NOT LIKE N''NT SERVICE\%'''
					ELSE N''
					END
				);;
	...
	SELECT @sysadmin = principal_id
	FROM @srv_principals
	WHERE [name] = N'sysadmin';;

	RAISERROR (
			'Windows users that are members of Windows group logins can be resolved using xp_logininfo:'
			,0
			,0
			)
	WITH NOWAIT
	...
	BEGIN
		...
		BEGIN
			...
			EXECUTE sys.xp_logininfo @acctname = @name
				,@option = 'members';
		END;
	...
	FROM (
		...
		WHERE account_name NOT IN (
				SELECT [name]
				FROM @srv_principals
				WHERE [type_desc] = N'WINDOWS_LOGIN'
				)
		) AS sub;;
	...
	JOIN @srv_principals AS m
		ON m.[type_desc] != 'SERVER_ROLE'
	WHERE r.[name] = 'public'
		AND m.principal_id >= 0;;
	...
	JOIN @srv_principals AS r
		ON xp.permission_path = r.[name]
	JOIN @srv_principals AS m
		ON xp.account_name = m.[name];;
	...
	SELECT DISTINCT wl.principal_id
		,N'WINDOWS_USER' AS [type_desc]
		,wl.[sid]
		,wl.[name]
	FROM @srv_principals AS wg
	JOIN @srv_members AS m
		ON wg.principal_id = m.role_principal_id
			AND m.member_principal_id < 0
	JOIN @srv_principals AS wl
		ON m.member_principal_id = wl.principal_id
	JOIN @db_principals AS dp
		ON dp.[sid] = wg.[sid];;
	...
	SELECT (
			SELECT MAX(principal_id)
			FROM @db_principals
			) + ROW_NUMBER() OVER (
			ORDER BY sp.principal_id
			) AS principal_id
		,REPLACE(sp.[type_desc], N'_LOGIN', N'_USER')
		,sp.[sid]
		,sp.[name]
		,dbo.principal_id
	FROM @srv_principals AS sp
	LEFT JOIN @db_principals AS db
		ON sp.[sid] = db.[sid]
	JOIN @db_principals AS dbo
		ON dbo.[name] = N'dbo'
	WHERE db.[sid] IS NULL
		AND (
			sp.principal_id IN (
				SELECT grantee_principal_id
				FROM sys.server_permissions
				WHERE [permission_name] = N'CONTROL SERVER'
				)
			OR sp.principal_id = (
				SELECT owner_sid
				FROM sys.databases
			...
			OR sp.principal_id IN (
				SELECT member_principal_id
				FROM sys.server_role_members
				WHERE role_principal_id = @sysadmin
				)
			);;
	...
	JOIN @db_principals AS m
		ON m.[type_desc] != 'DATABASE_ROLE'
	WHERE r.[name] = 'public';;
		...
		SELECT principal_id AS declared_principal_id
			...
			,CAST((
					CASE 
						WHEN [name] IN ('sa')
							THEN N'"' + [name] + N'"'
						ELSE LOWER(REPLACE([type_desc], N'_', N' ')) COLLATE database_default + N' "' + [name] + N'"'
						END
		...
		SELECT s_cte.declared_principal_id
			...
			,s_cte.[path] + ' -> ' + CAST((
					CASE 
						WHEN sp.[name] IN ('sa')
							THEN N'"' + sp.[name] + N'"'
						ELSE LOWER(REPLACE(sp.[type_desc], N'_', N' ')) COLLATE database_default + N' "' + sp.[name] + N'"'
						END
					) AS NVARCHAR(max)) AS [path]
		FROM @srv_members AS srm
		JOIN s_cte
			ON s_cte.effective_principal_id = srm.role_principal_id
		JOIN @srv_principals AS sp
			ON srm.member_principal_id = sp.principal_id
		WHERE s_cte.declared_name NOT LIKE 'NT AUTHORITY\%'
			AND sp.[name] NOT LIKE 'NT AUTHORITY\%'
			AND s_cte.declared_principal_id != sp.principal_id
		)
		,-- vain attempt to stop infinite recursion
	d_cte
	AS (
		--'Database principals (anchor)',0,0) WITH NOWAIT
		SELECT principal_id AS declared_principal_id
			,[type_desc] AS declared_type_desc
			,[sid] AS declared_sid
			,[name] AS declared_name
		...
		SELECT d_cte.declared_principal_id
			...
			,d_cte.[path] + ' -> ' + CAST((
					CASE 
						WHEN dp.[name] IN ('dbo')
							THEN N'"' + dp.[name] + N'"'
						ELSE LOWER(REPLACE(dp.[type_desc], N'_', N' ')) COLLATE database_default + N' "' + dp.[name] + N'"'
						END
					) AS NVARCHAR(max)) AS [path]
		FROM @db_members AS drm
		JOIN d_cte
			ON d_cte.effective_principal_id = drm.role_principal_id
		JOIN @db_principals AS dp
			ON drm.member_principal_id = dp.principal_id
		WHERE d_cte.declared_principal_id != dp.principal_id
		)
		,-- vain attempt to stop infinite recursion
	cte
	AS (
		--'Server principal hierarchy',0,0) WITH NOWAIT
		...
		--'Database principal hierarchy',0,0) WITH NOWAIT
		SELECT declared_principal_id
			,declared_type_desc
			,declared_sid
			,declared_name
		...
		FROM s_cte
		JOIN d_cte
			ON s_cte.effective_sid = d_cte.declared_sid
		)
	INSERT INTO @principals
	SELECT *
	FROM cte
	WHERE @principal IS NULL
		OR [path] LIKE N'%' + @principal + N'%';
		-------------------------------------------------------------------------------
		;
	...
	-------------------------------------------------------------------------------
	INSERT INTO @builtin_permissions (
		class
		,permission
		,declared_class
		,declared_permission
		)
	SELECT bip.class_desc AS class
		,bip.[permission_name] AS permission
		,x.declared_class
		,x.declared_permission_name
	...
	AS (
		...
		SELECT p.class
			...
			,CAST(cte.[path] + N' -> ' + (
					CASE 
						WHEN p.permission = p.declared_permission
							OR @permission_list = 1
							THEN N''
						ELSE p.permission + N' on '
						END
					) + p.class AS NVARCHAR(max)) AS [path]
			,cte.[level] + 1
		FROM @builtin_permissions AS p
		JOIN cte
			ON p.declared_class = cte.class
				AND p.declared_permission = cte.permission
		)
	--Save the results into temporary work table:
	INSERT INTO @permissions_temp (
		class
		,permission
		,declared_class
		,declared_permission
	...
	FROM (
		SELECT class
			...
			,ROW_NUMBER() OVER (
				...
				,permission
				,declared_class
				,declared_permission ORDER BY [level]
				) AS _rn
		FROM cte
		WHERE @permission IS NULL
			OR [path] LIKE N'%' + @permission + N'%'
			OR permission LIKE N'%' + @permission + N'%'
		) AS sub
	WHERE _rn = 1;;
	...
	RAISERROR (
			'If @permission_list is 1, compile a comma-separated list of permissions from the work table and insert into @permissions:'
			,0
			,0
			)
	WITH NOWAIT
	...
	SELECT class
		,SUBSTRING(CAST((
					SELECT N', ' + x.permission
					FROM @permissions_temp AS x
					WHERE x.declared_class = cte.declared_class
						AND x.declared_permission = cte.declared_permission
						AND x.class = cte.class
					ORDER BY x.permission
					FOR XML PATH('')
						,TYPE
					) AS NVARCHAR(max)), 3, 4000) AS permission
		,declared_class
		,declared_permission
		,MIN([path])
		,DENSE_RANK() OVER (
			PARTITION BY class
			,declared_class
			,declared_permission ORDER BY MIN([level])
			) AS [level]
	FROM @permissions_temp AS cte
	WHERE @permission_list = 1
		AND (
			@permission IS NULL
			OR [path] LIKE N'%' + @permission + N'%'
			)
	...
	-- SERVER
	SELECT NULL AS parent_class
		,CAST(NULL AS INT) AS parent_major_id
		,100 AS class
		,0 AS major_id
		,CAST(NULL AS INT) AS principal_id
		,'SERVER' AS class_desc
		,CAST(NULL AS NVARCHAR(max)) AS qualified_name
		,1 AS is_server_lvl
		,[objectType] = 'SERVER'
	...
	JOIN sys.availability_replicas agr
		ON agr.group_id = ag.group_id
	JOIN sys.dm_hadr_availability_replica_states AS agrs
		ON agrs.replica_id = agr.replica_id
			AND agrs.is_local = 1
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,31 AS class
		,property_list_id AS major_id
		,principal_id
		,'SEARCH PROPERTY LIST'
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,3 AS class
		,[schema_id] AS major_id
		,principal_id
		,'SCHEMA' AS class_desc
		,N'SCHEMA::' + QUOTENAME([name])
		,0 AS is_server_lvl
		,[objectType] = 'SCHEMA'
	...
	--OBJECT
	SELECT 3 AS parent_class
		,o.[schema_id] AS parent_major_id
		,1 AS class
	...
	SELECT 0 AS parent_class
		...
		,4 AS class
		,principal_id AS major_id
		,owning_principal_id AS principal_id
		,(
			CASE [type]
				WHEN 'A'
					THEN N'APPLICATION ROLE'
				WHEN 'R'
					THEN N'ROLE'
				ELSE N'USER'
				END
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,5 AS class
		,assembly_id AS major_id
		,principal_id
		,'ASSEMBLY'
		,N'ASSEMBLY::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	-- TYPE
	SELECT 3 AS parent_class
		,t.[schema_id] AS parent_major_id
		,6 AS class
		,t.user_type_id AS major_id
		,t.principal_id
		,'TYPE' AS class_desc
		,N'TYPE::' + QUOTENAME(s.[name]) + N'.' + QUOTENAME(t.[name])
		,0 AS is_server_lvl
	...
	SELECT 3 AS parent_class
		,x.[schema_id] AS parent_major_id
		,10 AS class
		,x.xml_collection_id AS major_id
		,x.principal_id
		,'XML SCHEMA COLLECTION' AS class_desc
		,N'XML SCHEMA COLLECTION::' + QUOTENAME(s.[name]) + N'.' + QUOTENAME(x.[name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,15 AS class
		,message_type_id AS major_id
		,principal_id
		,'MESSAGE TYPE'
		,N'MESSAGE TYPE::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,16 AS class
		,service_contract_id AS major_id
		,principal_id
		,'SERVICE CONTRACT'
		,N'SERVICE CONTRACT::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,17 AS class
		,service_id AS major_id
		,principal_id
		,'SERVICE'
		,N'SERVICE::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,18 AS class
		,remote_service_binding_id AS major_id
		,principal_id
		,'REMOTE SERVICE BINDING'
		,N'REMOTE SERVICE BINDING::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,19 AS class
		,route_id AS major_id
		,principal_id
		,'ROUTE'
		,N'ROUTE::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,23 AS class
		,fulltext_catalog_id AS major_id
		,principal_id
		,'FULLTEXT CATALOG'
		,N'CATALOG::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,29 AS class
		,stoplist_id AS major_id
		,principal_id
		,'FULLTEXT STOPLIST'
		,N'STOPLIST::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,24 AS class
		,symmetric_key_id AS major_id
		,principal_id
		,'SYMMETRIC KEY'
		,N'SYMMETRIC KEY::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,25 AS class
		,certificate_id AS major_id
		,principal_id
		,'CERTIFICATE'
		,N'CERTIFICATE::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	SELECT 0 AS parent_class
		,0 AS parent_major_id
		,26 AS class
		,asymmetric_key_id AS major_id
		,principal_id
		,'ASYMMETRIC_KEY'
		,N'ASYMMETRIC_KEY::' + QUOTENAME([name])
		,0 AS is_server_lvl
	...
	WITH cte
	AS (
		SELECT class AS parent_class
			,major_id AS parent_major_id
			,ISNULL(qualified_name, N'SERVER') AS parent_qualified_name
			,class
			,class_desc
			,major_id
			,qualified_name
			...
			,is_server_lvl
			,0 AS [level]
			,CAST(ISNULL(qualified_name, 'SERVER') AS NVARCHAR(max)) AS [path]
			,s.objectType
		...
		SELECT cte.parent_class
			,cte.parent_major_id
			,cte.parent_qualified_name
			,s.class
			,s.class_desc
			,s.major_id
			,s.qualified_name
			...
			,s.is_server_lvl
			,cte.[level] + 1
			,CAST(cte.[path] + N' -> ' + s.qualified_name AS NVARCHAR(max))
			,s.objectType
			,s.[objectTypeDescription]
		FROM cte
		JOIN @securables_temp AS s
			ON cte.class = s.parent_class
				AND cte.major_id = s.parent_major_id
		)
	...
	FROM (
		SELECT parent_class
			...
			,[path]
			,ROW_NUMBER() OVER (
				PARTITION BY parent_class
				,parent_major_id
				,class
				,major_id ORDER BY [level]
					,[path]
				) AS _rn
			,cte.objectType
			,cte.[objectTypeDescription]
		FROM cte
		WHERE @securable IS NULL
			OR REPLACE(REPLACE(qualified_name, N']', N''), N'[', N'') LIKE REPLACE(REPLACE(@securable, N']', N''), N'[', N'')
		) AS sub
	WHERE _rn = 1;
		-------------------------------------------------------------------------------
		;
	...
	JOIN sys.fn_builtin_permissions('SERVER') AS p
		ON sp.[name] = 'bulkadmin'
	...
	CROSS JOIN (
		...
		) AS s([state], state_desc)
	JOIN sys.fn_builtin_permissions('DATABASE') AS p
		ON dp.[name] = 'db_accessadmin'
			AND s.[state] = 'G'
			AND p.[permission_name] IN (
				'ALTER ANY USER'
				,'CREATE SCHEMA'
			...
			OR dp.[name] = 'db_datareader'
			AND s.[state] = 'G'
			AND p.[permission_name] = 'SELECT'
			OR dp.[name] = 'db_datawriter'
			AND s.[state] = 'G'
			AND p.[permission_name] IN (
				'INSERT'
				,'DELETE'
			...
			OR dp.[name] = 'db_denydatareader'
			AND s.[state] = 'D'
			AND p.[permission_name] = 'SELECT'
			OR dp.[name] = 'db_denydatawriter'
			AND s.[state] = 'D'
			AND p.[permission_name] IN (
				'INSERT'
				,'DELETE'
		...
		SELECT @xml = (
				SELECT perms.state_desc AS [@state]
					...
					(
						...
						WHERE perms.class_desc COLLATE database_default = p.declared_class
							AND perms.[permission_name] COLLATE database_default = p.declared_permission
					...
					(
						SELECT sec.class_desc AS [@class]
							...
							,(
								CASE 
									WHEN sec.class = sec.parent_class
										AND sec.major_id = sec.parent_major_id
										THEN 'declared'
									ELSE 'effective'
									END
								) AS [@inheritance]
						FROM @securables AS sec
						WHERE perms.class = sec.parent_class
							AND perms.major_id = sec.parent_major_id
						FOR XML PATH('securable')
							,TYPE
						) AS securables
					,
					...
					(
						SELECT grantee.effective_principal_id AS [@principal_id]
							...
							,(
								CASE 
									WHEN grantee.[level] = 0
										THEN 'declared'
									ELSE 'effective'
									END
								) AS [@inheritance]
						FROM @principals AS grantee
						WHERE grantee.declared_is_server_lvl = perms.grantee_is_server_lvl
							AND grantee.declared_principal_id = perms.grantee_principal_id
						ORDER BY grantee.[level]
						FOR XML PATH('principal')
							,TYPE
						) AS principals
					,
					...
					(
						CASE 
							--Explicit GRANT/DENY:
							WHEN perms.inheritance IS NULL
								THEN (
										CASE perms.[state]
											WHEN 'W'
												THEN N'GRANT'
											ELSE perms.state_desc
											END
										) + N' ' + perms.[permission_name] COLLATE database_default + ISNULL(N' ON ' + NULLIF(def_sec.parent_qualified_name, N'SERVER') + ISNULL(N'(' + QUOTENAME(col.[name] COLLATE database_default) + N')', N''), N'') + N' TO ' + QUOTENAME(def_grantee.declared_name) + (
										CASE 
											WHEN perms.[state] = 'W'
							...
							WHEN perms.inheritance = 'OWNER'
								THEN N'ALTER AUTHORIZATION ' + ISNULL(N' ON ' + def_sec.parent_qualified_name, N'') + N' TO ' + QUOTENAME(def_grantee.declared_name) COLLATE database_default
							END
						) AS [declared_permission/@sql]
				FROM @rules AS perms
				JOIN @principals AS def_grantee
					ON def_grantee.declared_is_server_lvl = perms.grantee_is_server_lvl
						AND def_grantee.declared_principal_id = perms.grantee_principal_id
						AND def_grantee.[level] = 0
				JOIN @permissions AS def_p
					ON perms.class_desc COLLATE database_default = (
							CASE 
								WHEN def_p.declared_class IN (
										N'USER'
										,N'ROLE'
										)
									THEN N'DATABASE_PRINCIPAL'
								ELSE def_p.declared_class
								END
							)
						AND perms.[permission_name] COLLATE database_default = def_p.declared_permission
						AND def_p.[level] = 0
				JOIN @securables AS def_sec
					ON perms.class = def_sec.parent_class
						AND perms.major_id = def_sec.parent_major_id
						AND def_p.class = def_sec.class_desc
						AND def_sec.class = def_sec.parent_class
						AND def_sec.major_id = def_sec.parent_major_id
				LEFT JOIN sys.columns AS col
					ON perms.grantee_is_server_lvl = 0
						AND perms.class_desc = 'OBJECT'
						AND perms.major_id = col.[object_id]
						AND perms.minor_id = col.column_id
				WHERE (
						@principal IS NULL
						OR def_grantee.effective_name LIKE @principal
						)
					AND (
						@securable IS NULL
						OR def_sec.qualified_name LIKE @securable
						)
					AND (
						@permission IS NULL
						OR perms.[permission_name] LIKE @permission
						)
				FOR XML PATH('permission')
					,TYPE
				);;
	...
	IF (@output_xml = 0)
					...
					WHEN perms.inheritance IS NULL
						THEN (
								CASE perms.[state]
									...
									ELSE perms.state_desc
									END
								) + N' ' + perms.[permission_name] COLLATE database_default + ISNULL(N' ON ' + NULLIF(sec.parent_qualified_name, N'SERVER') + ISNULL(N'(' + QUOTENAME(col.[name] COLLATE database_default) + N')', N''), N'') + N' TO ' + QUOTENAME(grantee.declared_name) + (
								CASE 
									WHEN perms.[state] = 'W'
										THEN N' WITH GRANT OPTION'
									ELSE N''
							...
							--Inferred CONTROL by ownership: ALTER AUTHORIZATION
					WHEN perms.inheritance = 'OWNER'
						THEN N'ALTER AUTHORIZATION ' + ISNULL(N' ON ' + sec.parent_qualified_name, N'') + N' TO ' + QUOTENAME(grantee.declared_name) COLLATE database_default
					END
				) AS declared_permission
			,
			...
			,(
				CASE 
					WHEN sec.class_desc IN (
						...
						THEN 1
					WHEN sec.class_desc = 'OBJECT'
						THEN tblSO.[allowed]
					END
				) AS [allowed]
			,sec.class_desc
		FROM @rules AS perms
		JOIN @principals AS grantee
			ON grantee.declared_is_server_lvl = perms.grantee_is_server_lvl
				AND grantee.declared_principal_id = perms.grantee_principal_id
		LEFT JOIN sys.columns AS col
			ON perms.grantee_is_server_lvl = 0
				AND perms.class_desc = 'OBJECT'
		...
		JOIN @permissions AS p
			ON perms.class_desc COLLATE database_default = (
					CASE 
						WHEN p.declared_class IN (
								N'USER'
								,N'ROLE'
								)
							THEN N'DATABASE_PRINCIPAL'
						ELSE p.declared_class
						END
					)
				AND perms.[permission_name] COLLATE database_default = p.declared_permission
				AND (
					...
					AND grantee.effective_is_server_lvl = 1
					OR p.class NOT IN (
						'SERVER'
						,'SERVER_PRINCIPAL'
						,'ENDPOINT'
						)
					AND grantee.effective_is_server_lvl = 0
					)
		JOIN @securables AS sec
			ON perms.class = sec.parent_class
				AND perms.major_id = sec.parent_major_id
				AND p.class = sec.class_desc
		LEFT JOIN @tblSupportObject AS tblSO
			ON [sec].[class_desc] = 'OBJECT'
				AND [sec].[objectType] = tblSO.[type]
				AND p.[permission] = tblSO.[operation]
		WHERE (
				@principal IS NULL
				OR grantee.effective_name LIKE @principal
				)
			AND (
				@securable IS NULL
				OR sec.qualified_name LIKE @securable
				)
			AND (
				@permission IS NULL
				OR perms.[permission_name] LIKE @permission
				OR p.permission LIKE @permission
				)
...
IF DB_NAME() = 'master'
AND CAST(SERVERPROPERTY('Edition') AS VARCHAR(100)) NOT LIKE '%Azure%'
BEGIN
